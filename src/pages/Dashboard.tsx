...
export function Dashboard() {
  const [accessLevel, setAccessLevel] = useState(() => sessionStorage.getItem('dashboard_auth') || 'none');
  const [passwordInput, setPasswordInput] = useState('');
  const [showPassword, setShowPassword] = useState(false);

  const isAuthenticated = accessLevel !== 'none';

  const handleLogin = (e: React.FormEvent) => {
    e.preventDefault();
    if (passwordInput === 'gurvinderIsAdmin' || passwordInput === 'gurvindersingh85698') {
      setAccessLevel('admin');
      sessionStorage.setItem('dashboard_auth', 'admin');
    } else if (passwordInput === 'gurvinderbrar89013') {
      setAccessLevel('featured');
      sessionStorage.setItem('dashboard_auth', 'featured');
    } else if (passwordInput === 'gurvindersingh71632') {
      setAccessLevel('upload');
      sessionStorage.setItem('dashboard_auth', 'upload');
    } else if (passwordInput === 'gurvinder52254singh') {
      setAccessLevel('list');
      sessionStorage.setItem('dashboard_auth', 'list');
    } else {
  ...
  return (
    <div className="container" style={{ paddingTop: '2rem', paddingBottom: '2rem' }}>
      ...
      <div className="dashboard-header">
        ...
        <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
          <button
            ...
            onClick={() => { setAccessLevel('none'); sessionStorage.removeItem('dashboard_auth'); }}
            style={{ padding: '0.4rem 1rem', borderColor: 'rgba(255,255,255,0.2)' }}
          >
            Logout
          </button>
          {(accessLevel === 'admin' || accessLevel === 'upload') && !isAdding && (
      ...
      {(accessLevel === 'admin' || accessLevel === 'featured') && (
      ...
      {(accessLevel === 'admin' || accessLevel === 'list') && (
